...
def autosave_section(section, code):
    ...
    doc = frappe.get_doc(doctype="Code Revision",
                         section=section, code=code, author=frappe.session.user)
    doc.insert()
...
def validate_billing_access(billing_type, name):
    ...
    if access and billing_type == "course":
        ...
        membership = frappe.db.exists("LMS Batch Enrollment", {
                                      "member": frappe.session.user, "batch": name})
        if membership:
            access = False
        ...
        number_of_students = frappe.db.count(
            "LMS Batch Enrollment", {"batch": name})
...
def get_job_opportunities(filters=None, orFilters=None):
    ...
    for job in jobs:
        ...
        job.applicants = frappe.db.count(
            "LMS Job Application", {"job": job.name})
    return jobs
...
def get_chart_details():
    ...
    details.certifications = frappe.db.count(
        "LMS Certificate", {"published": 1})
    return details
...
def get_certified_participants(filters=None, start=0, page_length=100):
    ...
    for participant in participants:
        count = frappe.db.count(
            "LMS Certificate", {"member": participant.member})
...
def get_count_of_certified_members(filters=None):
    ...
    query = (
        frappe.qb.from_(Certificate).select(
            Certificate.member).distinct().where(Certificate.published == 1)
    )
    ...
    if filters:
        for field, value in filters.items():
            if field == "category":
                query = query.where(
                    Certificate.course_title.like(
                        f"%{value}%") | Certificate.batch_title.like(f"%{value}%")
                )
...
def get_assigned_badges(member):
    ...
    for badge in assigned_badges:
        badge.update(frappe.db.get_value(
            "LMS Badge", badge.badge, ["name", "title", "image"]))
...
def update_source_chapter(lesson, chapter, idx, hasMoved=False):
    ...
    if not hasMoved:
        frappe.db.delete("Lesson Reference", {
                         "parent": chapter, "lesson": lesson})
    else:
...
def update_index(lessons, chapter):
    for row in lessons:
        frappe.db.set_value(
            "Lesson Reference", {
                "lesson": row, "parent": chapter}, "idx", lessons.index(row) + 1
        )
...
def update_chapter_index(chapter, course, idx):
    ...
    for i, chapter_name in enumerate(chapters):
        frappe.db.set_value("Chapter Reference", {
                            "chapter": chapter_name, "parent": course}, "idx", i + 1)
...
def check_app_permission():
    ...
    lms_roles = ["Moderator", "Course Creator",
...
def save_certificate_details(
    ...
    certificate = frappe.db.exists(
        "LMS Certificate", {"member": member, "course": course})
...
def get_payment_gateway_details(payment_gateway):
    ...
    if gateway.gateway_controller is None:
        ...
        try:
            data = frappe.get_doc(gateway.gateway_settings,
                                  gateway.gateway_controller).as_dict()
...
def update_course_statistics():
    ...
    for course in courses:
        ...
        avg_rating = flt(avg_rating, frappe.get_system_settings(
...
def delete_course(course):
    chapters = frappe.get_all(
        "Course Chapter", {"course": course}, pluck="name")
    ...
    for chapter in chapters:
        lessons = frappe.get_all(
            "Course Lesson", {"chapter": chapter}, pluck="name")
...
def delete_batch(batch):
    ...
    frappe.db.delete("Batch Course", {
                     "parent": batch, "parenttype": "LMS Batch"})
    frappe.db.delete("LMS Assessment", {
                     "parent": batch, "parenttype": "LMS Batch"})
    frappe.db.delete("LMS Batch Timetable", {
                     "parent": batch, "parenttype": "LMS Batch"})
...
def check_for_malicious_code(zip_path):
    suspicious_patterns = [
        ...
        r'on(click|load|mouseover|error|submit|focus|blur|change|keyup|keydown|keypress|resize)=".*?"',
                ...
                with zf.open(file_name) as file:
                    ...
                    for pattern in suspicious_patterns:
                        if re.search(pattern, content):
                            frappe.throw(_("Suspicious pattern found in {0}: {1}").format(
                                file_name, pattern))
    ...
    if manifest_file:
        ...
        if launch_file:
            launch_file = os.path.join(
                os.path.dirname(manifest_file), launch_file)
...
def mark_lesson_progress(course, chapter_number, lesson_number):
    ...
    lesson_name = frappe.get_value(
        "Lesson Reference", {"parent": chapter_name,
                             "idx": lesson_number}, "lesson"
    )
...
def get_heatmap_data(member=None, base_days=200):
    ...
    base_date, start_date, number_of_days, days = calculate_date_ranges(
        base_days)
...
def cancel_evaluation(evaluation):
    ...
    frappe.db.set_value("LMS Certificate Request",
                        evaluation.name, "status", "Cancelled")
    ...
    for event in events:
        info = frappe.db.get_value("Event", event.parent, [
                                   "starts_on", "subject"], as_dict=1)
        ...
        if date == str(evaluation.date.format("YYYY-MM-DD")) and evaluation.member_name in info.subject:
            ...
            if communication:
                frappe.delete_doc(
                    "Communication", communication, ignore_permissions=True)

            frappe.delete_doc("Event Participants",
                              event.name, ignore_permissions=True)
...
def get_certification_details(course):
    ...
    paid_certificate = frappe.db.get_value(
        "LMS Course", course, "paid_certificate")
...
def delete_evaluator(evaluator):
    ...
    frappe.db.delete(
        "Has Role", {"parent": evaluator, "role": "Batch Evaluator"})
    ...
    for tag in meta_tags:
        ...
        if existing_tag:
            if not tag.get("value"):
                ...
                continue
            frappe.db.set_value("Website Meta Tag",
...
def update_exercise_submission(submission, code, test_cases):
    ...
    frappe.db.set_value("LMS Programming Exercise Submission",
                        submission, {"status": status, "code": code})
